...
Um das heterogene Multiagentensystem zu synchronisieren, müssen die Agenten das \emph{kooperative Interne-Modell-Pronzip} erfüllen:
...
Es wird nun ein Beobachter entworfen, der für die Agenten die Führungsagentenzustände rekonstruiert.
...
Führungsagenten rekonstruieren ($\hat{v_i}$).
Bei informierten Agenten ist das ein klassischer Beobachter:

\begin{equation}
    \dot{\hat{v}}_i = S_w \hat{v}_i + L_i a_{i0} (w-\hat{w}_i)
\end{equation}
(Die Beobachterrückführmatrizen $L_i$ lassen sich unterschiedlich wählen.)

Bei nichtinformierten Agenten ein kooperativer Führungsbeobachter, der dann durch diffusive Kopplung mit den anderen Agenten kommuniziert:

\begin{equation}
    \dot{\hat{v}}_i = S_w \hat{v}_i + L \sum_{j=1}^N a_{ij} (\hat{w}_j - \hat{w}_i)
\end{equation}
(Gemeinsame Beobachterrückführmatrix $L$.)
\todo[inline]{i-j oder j-i? skript und übung unterschiedlich}
...
\begin{equation}
    L_{\overbar{\mathcal{G}}} = \begin{bmatrix}
        ...
        0_{N-M \times 1} & L_{32} & L_{33}
    \end{bmatrix}
\end{equation}

Dabei ist $M$ die Anzahl informierter Agenten, $N$ die Anzahl Agenten, und $N-M$ die Anzahl
Nichtinformierter Agenten.
\begin{itemize}
    ...
    \item $L_{33}$: Verbindungen zwischen nichtinformierten Agenten
\end{itemize}
